...
resource "aws_ecs_service" "service" {
  ...
  # Sử dụng khi dùng chế độ CD với CodeDeploy (cho phép cập nhật ECS service)
  dynamic "deployment_controller" {
    for_each = var.revision_source == true ? [1] : []
    content {
  ...
  dynamic "load_balancer" {
    ...
    content {
      target_group_arn = var.green_target_arn
      container_name = var.green_containers_name
      container_port = var.green_containers_port
    }
  }
}
