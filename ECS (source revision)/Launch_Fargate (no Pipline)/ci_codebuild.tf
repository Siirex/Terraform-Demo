...
# ---------------------------------------------------
# 2 Lambda function: call CloudWatch Logs + CodeBuild
...
data "archive_file" "function_build" {
  type = "zip"
  source_dir = "../Local Repostory/code-function"
  output_path = "../Local Repostory/code-function/function_codebuild.zip"
}
*/
...
module "s3" {
  ...
  bucket_function_acl = var.bucket_function_acl
...
resource "aws_cloudwatch_log_group" "fucntion_build" {
  name = "/aws/lambda/${var.fucntion_build_name}"
  retention_in_days = 1
}
...
module "function" {
  ...
  # Function call CodeBuild (trigger by CodeCommit)
  function_build = var.fucntion_build_name
  role_build = module.role.role_function_build
  handler_build = var.hander_function_build
  runtime_build = var.runtime_function_build
  # file_function_build = "${data.archive_file.function_build.output_path}"
  ...
  bucket_function_build_key = var.key_store_function_build

  # Permission invoke Function for CodeCommit trigger
  statement_allow = "AllowExecutionFromCodeCommit"
  action_allow = "lambda:InvokeFunction"
  principal_allow = "codecommit.amazonaws.com"
  codecommit_repo_arn = module.repo_app.repo_arn
}
...
module "repo_app" {
  ...
  use_pipline = false
  trigger_function_build_name = "terraform_trigger_lambda_calling_CodeBuild"
  function_build_arn = module.function.funtion_build_arn[0] //function 'build'
  codebuild_name = var.codebuild_project_name //module.codebuild.build_project_name
}
...
data "template_file" "buildspec" {
  template = file("../Local Repostory/Configuration CI&CD/buildspec.yml")
  vars = {
    acc_id = var.acc_id
    region = var.region
